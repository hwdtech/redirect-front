...
class MainLinkForm extends PureComponent {
  constructor(props) {
    super(props);
    let state = { title: '', defaultLink: '' };
    console.log(props);
    if (props.formContent) {
      state = props.formContent;
    }

    this.state = state;
  }
  render() {
    const { onClick, onCancelClick, viewContent, selected, inputMode, validate, validateState } = this.props;
...
MainLinkForm.propTypes = {
  ...
  formContent: PropTypes.shape.isRequired,
};
